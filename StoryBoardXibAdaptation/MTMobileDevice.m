...
static NSUInteger MTSimulatorType = simulator;
static NSDictionary *deviceMap;

@implementation MTMobileDevice
...
+ (BOOL)isOneOfThem:(MTMobileDeviceType)firstType, ... {
    
    MTMobileDeviceType deviceType = [self deviceType];
    
    if ((deviceType == simulatori386 || deviceType == simulatorx86_64)
        && MTSimulatorType != simulator) {
        deviceType = MTSimulatorType;
    }
    ...
    [self emurate:firstType list:types
           action:^(MTMobileDeviceType type,BOOL *_break) {
...
+ (BOOL)type:(MTMobileDeviceType)originType isKindOfCommonType:(MTMobileDeviceType)commonType {
...
+ (void)emurate:(MTMobileDeviceType)firstType list:(va_list)types action:(void (^)(MTMobileDeviceType,BOOL *_break))action {
    ...
    MTMobileDeviceType nextType;
    while ((nextType = va_arg(types, MTMobileDeviceType))) {
